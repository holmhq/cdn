...
/* The plate fills the host's height, so charts laid out in a stretched grid/flex
   row (align-items:stretch) render as equal-height cards. height:100% resolves
   only when the host is given a definite height (e.g. a stretched grid item);
   in normal flow it collapses to auto, so standalone charts stay content-sized
   and nothing changes. Works whether the host computes to block or flex (a
   consumer may set either from the outer page, which overrides :host). */
.plate {
