// flow-globe v0.0.2 — configurable animated geospatial flow map.
//
...
const ESM_URLS = Object.freeze({
  core: 'https://unpkg.com/@amcharts/amcharts5@5.18.0/index.js?module',
  map: 'https://unpkg.com/@amcharts/amcharts5@5.18.0/map.js?module',
  geodata: 'https://unpkg.com/@amcharts/amcharts5-geodata@5.1.5/worldLow.js?module',
  animated: 'https://unpkg.com/@amcharts/amcharts5@5.18.0/themes/Animated.js?module',
});
...
export function loadAmCharts() {
  ...
  if (!loadPromise) {
    loadPromise = loadAmChartsESM().catch(() => loadAmChartsGlobals());
  }
  return loadPromise;
}

async function loadAmChartsESM() {
  const [am5, am5map, geodata, animated] = await Promise.all([
    import(ESM_URLS.core),
    import(ESM_URLS.map),
    import(ESM_URLS.geodata),
    import(ESM_URLS.animated),
  ]);
  return {
    am5,
    am5map,
    am5geodata_worldLow: geodata.default || geodata,
    am5themes_Animated: animated.default || animated,
  };
}

function loadAmChartsGlobals() {
  return SCRIPT_URLS.reduce((promise, url) => promise.then(() => loadScript(url)), Promise.resolve())
    .then(() => {
